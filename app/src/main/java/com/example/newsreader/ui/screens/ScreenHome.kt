...
@Composable
fun ScreenHome(parentNavController: NavHostController, viewModel: DataViewModel) {

    val pagingDataState = viewModel.pagingDataArticles.collectAsLazyPagingItems()

    when (pagingDataState.loadState.refresh) {
        is LoadState.Loading -> PlaceHolder()
        is LoadState.Error -> ErrorStateView(modifier = Modifier.fillMaxSize()) { pagingDataState.retry() }
            ...
            LazyColumn(
                ...
                content = {
                    items(pagingDataState.itemCount, key = {
                        ...
                        CardItem(result = item, parentNavController = parentNavController)
                    }
    ...
    viewModel.refreshDataArticles.observe(LocalLifecycleOwner.current) {
        if (it) {
            pagingDataState.refresh()
            viewModel.refreshDataArticles.value = false
        }
    }
}
...
@Composable
fun ErrorStateView(modifier: Modifier, retryAction: () -> Unit) {
...
@Composable
fun CardItem(result: Result, parentNavController: NavHostController) {
        ...
        Row {
            Column(Modifier.weight(1F)) {
                AsyncImage(
                    modifier = Modifier
                        .height(100.dp)
                        .width(100.dp), model = result.Image, contentDescription = null
                )
            }
            Column(
                modifier = Modifier
